...
Проверяет:
  1. Сохранение квазинейтральности (Σ z_k c_k ≈ 0)
  2. Устойчивость при dt = 1e-3
  3. Отсутствие отрицательных концентраций
  4. Больцмановское равновесие с мембранным потенциалом
  5. Спайк-отклик на скачок граничных концентраций
"""
...
# --- Ионы нейрона (безразмерные) ---
# Концентрации: снаружи (c_left) и внутри (c_right)
# Na: снаружи 145, внутри 10
# K:  снаружи 4,   внутри 140
# Cl: снаружи 110, внутри 110 (пассивный)
# Ca: снаружи 2.5, внутри 0.1
IONS = [
...
def test_charge_conservation():
    """Тест 2: Сохранение заряда (с учётом граничных условий Дирихле)."""
    ...
    # С граничными условиями Дирихле заряд не сохраняется строго —
    # но изменение должно быть монотонным (источник/сток на границах).
    Q0 = bio.total_charge()
    ...
    # Проверяем, что заряд меняется плавно (нет взрывов)
...
def test_boltzmann_equilibrium():
    ...
    # Линейный потенциал 0 -> -1.8 (как в модуле)
    bio = BiologicalDiffusion(
        graph,
        [
    ...
    # Устанавливаем граничные значения в больцмановские
    ...
    for step in range(100000):
        bio.step()
        c_prev = bio.c.copy() if step == 0 else c_prev
...
def test_spike_response():
    """Тест 4: Отклик на скачок граничных концентраций (имитация спайка).

    Меняем c_left для Na на 200 (деполяризация),
    проверяем что концентрации реагируют плавно.
    """
    ...
    Na_before = bio.c[0, N // 2] if (N := bio.N) else 0

    # Скачок: Na снаружи 145 -> 200 (деполяризация)
    ...
    if bio.c.min() < -1e-6:
        ...
        passed = False
    if abs(dNa) < 1e-6:
        print("❌ FAIL: No response to spike — system not reacting")
        passed = False
